...
async function initialize() {
  ...
  render();   // category combobox removed
}
...
function setupMainFilters() {
  $$('.type-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      e.target.classList.remove('bg-gray-300', 'dark:bg-gray-700');
      ...
      const hasPrice  = !state.type.includes('YouTube') && !state.type.includes('Website');
      const hasPop    = !isYT;

      langFilterContainer.classList.toggle('hidden', !isYT);
      ...
      $('.sort-option-pop').classList.toggle('hidden', !hasPop);
...
function setupLanguageFilters() {
  ...
  langFilters.innerHTML = [...allLangs].sort().map(l => `
  ...
  $$('.lang-checkbox').forEach(box =>
    box.addEventListener('change', e => {
      ...
      e.target.checked ? state.languages.add(lang) : state.languages.delete(lang);
      state.currentPage = 1;
      render();
    })
  );
...
function setupPlatformFilters() {
  const allPlats = new Set(allData.flatMap(i => i.platforms || []));
  platformFilters.innerHTML = [...allPlats].sort().map(p => `
    <label class="flex items-center gap-2 cursor-pointer">
...
function renderPagination(totalPages) {
  ...
  const createBtn = (label, pg, disabled = false) => {
  ...
  paginationControls.appendChild(createBtn('Prev', state.currentPage - 1, state.currentPage === 1));
  ...
  paginationControls.appendChild(createBtn('Next', state.currentPage + 1, state.currentPage === totalPages));
}
