...
const state = {
  ...
  musicDawCategories: new Set(), // Add this for Music DAW categories
  sortBy: 'default',
  currentPage: 1,
};
...
function setupMainFilters() {
  $$('.type-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      // Clear music DAW categories when switching away from Music Production DAWs
      if (state.type !== 'Music Production DAWs') {
      ...
      const isMusicDAW = state.type === 'Music Production DAWs';
      ...
      // Show music DAW category filter only for Music Production DAWs
      if (isMusicDAW) {
...
function restorePreviousState() {
  ...
  // Clear music DAW categories if we're not on Music Production DAWs
  if (state.type !== 'Music Production DAWs') {
    state.musicDawCategories.clear();
    $$('.music-daw-category-checkbox').forEach(cb => cb.checked = false);
  }
...
function card(t) {
  ...
  const popularityStats = t.popularityScore && state.type === 'Music Production DAWs'
...
/* ---------- RENDER ---------- */
function render() {
  // TEMPORARY DEBUG - Add this at the start of render()
  ...
  let filtered = allData.filter(item => {
    ...
    console.log('Item:', item.name, {
      typeMatch, tierMatch, catMatch, queryMatch, 
      langMatch, platMatch, courseCatMatch, websiteCatMatch, musicDawCatMatch,
      type: item.type,
...
(async () => {
  try {
    ...
    setupMusicDawCategoryFilters(); // Initialize Music DAW category filters
    setupSearch();
    setupSorting();
    render();
  } catch (error) {
