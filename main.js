...
/* ---------- THEME (works with CDN) ---------- */
function installThemeToggle() {
  const apply = () => {
    const wantDark = localStorage.theme === 'dark' ||
                    (!('theme' in localStorage) &&
                     window.matchMedia('(prefers-color-scheme: dark)').matches);

    // 1.  put / remove the class
    document.documentElement.classList.toggle('dark', wantDark);

    // 2.  re-build the whole CSS (forces CDN to notice)
    tailwind.refresh();

    // 3.  swap icons
    $('#icon-light').classList.toggle('hidden', wantDark);
    $('#icon-dark').classList.toggle('hidden', !wantDark);
    return wantDark;
  };
