...
function card(t) {
  ...
  if (!t || !t.name) {
    console.warn('Invalid item in card template:', t);
    return '<div class="hidden">Invalid item</div>';
  }
  ...
  <div class="block rounded-lg theme-card p-4 shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col">
    <div class="flex-grow">
      ...
      <p class="text-sm theme-text-secondary mb-2">${t.description || 'No description available'}</p>
      ${pricingInfo}
      ${ytStats}
      ${popularityStats}
    ...
    <div class="mt-auto pt-4 border-t theme-border">
      <div class="flex justify-between items-center">
        <p class="text-xs theme-text-secondary">${t.type || 'Unknown type'}${t.category ? ` • ${t.category}` : ''}</p>
...
/* ------------------------------------------------------------------ */
/*  PLATFORM FILTER – RESTRICTED & AUTO-CLEAR  (with counts)          */
/* ------------------------------------------------------------------ */
function setupPlatformFilters() {
  const allPlatforms = new Set(allData.flatMap(i => i.platforms || []));
  const whitelist = {
...
function render() {
  ...
  // IMPROVED: Enhanced sorting with better safety checks
  const sorted = [...filtered].sort((a, b) => {
    // Debug logging to check what's being sorted
    console.log('Sorting:', state.sortBy, 'a:', a.name, 'b:', b.name);
    ...
    switch (state.sortBy) {
      ...
      case 'name_desc': 
        const nameA2 = (a.name || '').toString().toLowerCase();
        const nameB2 = (b.name || '').toString().toLowerCase();
  ...
  // Debug: Check if we have any undefined items after sorting
  const undefinedItems = sorted.filter(item => !item.name);
  if (undefinedItems.length > 0) {
    console.warn('Found items with undefined names:', undefinedItems);
  }
