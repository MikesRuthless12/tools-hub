...
function getFilteredCount() {
  ...
  return allData.filter(item => {
    // BUG FIX: Skip items without a valid name
...
async function init() {
  try {
    ...
    // BUG FIX: Filter out any items that don't have a name property
    allData = allData.filter(item => item && item.name);
    ...
    $('#grid').innerHTML = '<p class="col-span-full text-center text-red-500">Failed to load data.</p>';
  }
}
