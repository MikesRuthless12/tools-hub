...
/* ---------- MUSIC DAW CATEGORY CHECKER ---------- */
function checkMusicDawCategory(item, selectedCategories) {
  if (item.type !== 'Music DAW') return false;
  
  // Use the dawCategories field if it exists
  const dawCategories = item.dawCategories || [];
  
  // Check if any of the selected categories match the DAW's categories
  return selectedCategories.some(cat => dawCategories.includes(cat));
}
...
function getFilteredCount() {
  ...
  return allData.filter(item => {
    ...
    // Use the new Music DAW category checking function
    const musicDawCatMatch = musicDawCatArr.length === 0 || checkMusicDawCategory(item, musicDawCatArr);
