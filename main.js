...
function setupMainFilters() {
  $$('.type-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      $$('.type-btn').forEach(b => b.classList.add('bg-gray-300'));
...
function card(t) {
  ...
  const langHtml = (t.languages  || []).map(l => `<span class="text-xs font-semibold px-2 py-0.5 rounded bg-gray-200 text-gray-700">${l}</span>`).join(' ');
  const platHtml = (t.platforms || []).map(p => `<span class="text-xs font-semibold px-2 py-0.5 rounded bg-gray-200 text-gray-700">${p}</span>`).join(' ');

  const ytStats = t.type && t.type.includes('YouTube') && t.subscribers && t.monthlyViews
    ? `<p class="text-sm text-gray-500 mb-2">Subscribers: <strong>${t.subscribers}</strong> · Monthly Views: <strong>${t.monthlyViews}</strong></p>`
    : '';

  return `
  <div class="block rounded-lg card-style p-4 shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col">
    <div class="flex-grow">
...
function renderPagination(totalPages) {
  ...
  const btn = (label, page, disabled = false) => {
    ...
    b.className   = disabled
      ...
      : 'px-3 py-1 rounded border card-style border-gray-300 hover:bg-gray-100';
...
function render() {
  ...
  $('#grid').innerHTML = page.map(card).join('') ||
                   `<p class="col-span-full text-center text-gray-600">No results match your filters.</p>`;
