...
function installThemeToggle() {
  // apply remembered / system preference
  const apply = () => {
  ...
  themeBtn.addEventListener('click', () => {
    const isDark = apply();                       // current state
    localStorage.theme = isDark ? 'light' : 'dark'; // flip
    apply();                                      // re-apply
  });
}
...
function setupMainFilters() {
  $$('.type-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      langFilterContainer.classList.toggle('hidden', !isYT);   // << hide langs for any YouTube
