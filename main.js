...
function installThemeToggle() {
  const apply = () => {
    ...
    document.documentElement.classList.toggle('dark', dark);   // ⬅️ actually paints dark
    $('#icon-light').classList.toggle('hidden', dark);
    $('#icon-dark').classList.toggle('hidden', !dark);
    return dark;
  };
  apply();                                    // first paint
  $('#theme-toggle').addEventListener('click', () => {
    localStorage.theme = apply() ? 'light' : 'dark'; // store opposite
    apply();                                // re-apply classes
  });
}
