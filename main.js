...
function setupMainFilters() {
  $$('.type-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      const isMusic    = state.type.includes('Music Production') || state.type === 'Music Samples' || state.type === 'Music DAW' || state.type === 'Music VST';
      ...
      const hasPlat  = ['IDE','Graphics Program','Graphics Utility',
                        'Music DAW','Music Production VSTs', 'Music VST'].includes(state.type);
      ...
      const showPopularity = state.type === 'Music Samples' || state.type === 'Music VST' || state.type === 'Music DAW';
...
function card(t) {
  ...
  const popularityStats = t.popularityScore && (state.type === 'Music DAW' || state.type === 'Music Samples' || state.type === 'Music VST')
...
function render() {
  ...
  let filtered = allData.filter(item => {
    ...
    // Search query matching - FIXED with proper null checks
    const queryMatch = !state.query || 
      (item.name && typeof item.name === 'string' && item.name.toLowerCase().includes(state.query)) ||
      (item.description && typeof item.description === 'string' && item.description.toLowerCase().includes(state.query));
