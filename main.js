...
function setupMainFilters() {
  $$('.type-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      const hideLang   = isYT || isAI || isGraphics || isMusic || isDevCourse; // Added isDevCourse here
      ...
      $('#course-category-filter-container').classList.toggle('hidden', !isDevCourse);
...
function getFilteredCount() {
  ...
  return allData.filter(item => {
    ...
    return typeMatch && tierMatch && catMatch && queryMatch && langMatch && platMatch && courseCatMatch;
  }).length;
}
