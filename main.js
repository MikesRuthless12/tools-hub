...
const state = {
  ...
  websiteCategories: new Set(), // Add this for Dev Website categories
  sortBy: 'default',
  currentPage: 1,
};
...
function setupMainFilters() {
  $$('.type-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      const isDevWebsite = state.type === 'Dev Website'; // Add this
      ...
      // Show website category filter only for Dev Website
      if (isDevWebsite) {
...
function getFilteredCount() {
  ...
  const websiteCatArr = [...state.websiteCategories]; // Add this
  ...
  return allData.filter(item => {
    ...
    const websiteCatMatch = websiteCatArr.length === 0 || // Add this
          (item.category && websiteCatArr.includes(item.category));
    return typeMatch && tierMatch && catMatch && queryMatch && langMatch && platMatch && courseCatMatch && websiteCatMatch;
  }).length;
}
...
(async () => {
  ...
  setupWebsiteCategoryFilters(); // Add this
  setupSearch();
  setupSorting();
  render();
})();